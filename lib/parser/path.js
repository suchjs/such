...
var parser = {
    config: {
        ...
        endTag: [],
        separator: ',',
        pattern: new RegExp("^(?:(\\.(?:\\/(?=\\.)|))?((?:\\.\\.(?:\\/(?=\\.)|))*)|<(\\w+)>)((?:\\/(?:\\\\.|[^,\\\\" + config_1.encodeSplitor + "\\/])+)+)"),
        rule: new RegExp("^&(?:(?:(?:\\.(?:\\/(?=\\.)|))?(?:\\.\\.(?:\\/(?=\\.)|))*|<(\\w+)>)(?:\\/(?:\\\\.|[^,\\\\" + config_1.encodeSplitor + "\\/])+)+?(?:,(?=\\/|\\.)|(?=$|" + config_1.encodeSplitor + ")))+"),
    },
    parse: function () {
        ...
        patterns.forEach(function (match) {
            var fullpath = match[0], lookParent = match[1], lookDepth = match[2], variable = match[3], curPath = match[4];
