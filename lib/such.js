...
var capitalize = utils.capitalize, isFn = utils.isFn, isOptional = utils.isOptional, makeRandom = utils.makeRandom, map = utils.map, typeOf = utils.typeOf, deepCopy = utils.deepCopy;
...
var Such = (function () {
    ...
    Such.config = function (config) {
        ...
        var fnHashs = { parsers: 'parser', types: 'define', globals: 'assign' };
        var lastConf = {};
        var such = Such;
        ...
        Object.keys(lastConf).map(function (key) {
            ...
            var fnName = fnHashs[key] || key;
            Object.keys(conf).map(function (name) {
                var args = typeOf(conf[name]) === 'Array' ? conf[name] : [conf[name]];
                such[fnName].apply(such, [name].concat(args));
            });
        });
    };
    ...
    Such.define = function (type) {
        ...
        var param = config.param, init = config.init, generateFn = config.generateFn, generate = config.generate;
