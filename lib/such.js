...
var capitalize = utils.capitalize, isFn = utils.isFn, isOptional = utils.isOptional, makeRandom = utils.makeRandom, map = utils.map, typeOf = utils.typeOf, deepCopy = utils.deepCopy, isNoEmptyObject = utils.isNoEmptyObject;
    ...
    Mocker.prototype.mock = function (dpath) {
        ...
        if (this.isRoot) {
            if (this.promises.length) {
                ...
                return this.result = Promise.all(queues_1).then(function (results) {
                    results.map(function (res, i) {
                        _this.datas.set(dpaths_1[i], res);
                    });
                    return _this.datas.get([]);
                });
            }
        }
        else {
