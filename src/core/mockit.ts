...
import { SpecialType, TOverrideParams, TSuchInject } from "../types/instance";
...
const getNsValues = (callerNs: string, ownerNs: string): {
		...
		$config: function (this: Mockit, $config: IPPConfig = {}): IPPConfig {
			...
			Object.keys(configOptions).map((key: string) => {
				...
				let validator = (target: unknown) => {
					...
					const allTypes = isArray(type) ? type : (type ? [type] : []);
					if (allTypes.length) {
						let flag = false;
						allTypes.map((cur) => {
				...
				if (isObject(cur)) {
					...
					type = cur.type;
					validator = isFn<typeof validator>(
						cur.validator,
					) ? cur.validator : validator;
				}
				if (hasKey) {
					if (!validator.call(this, last[key])) {
						throw new Error(
							`the config of "${key}"'s value ${last[
								key
			...
			Object.keys(options.param).map((key) => {
				if (hasOwn(this.params, key) && allowdOverrideParams.includes(key)) {
					...
					params[key] = Object.assign(
						{},
						origParam,
						(mutates.includes(key) ? ruleFns[key](curParam) : curParam),
					);
				} else {
					// eslint-disable-next-line no-console
					console.warn(
						`You set a '${key}' param in instanceOptions's params not allowed.`,
					);
				}
			});
			return params;
