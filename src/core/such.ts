		...
		if (isArray(target)) {
			...
			if (hasLength) {
				...
				const makeArrFn = (
					...
					const makeInstance =
						instance instanceof Mocker ? (_i: number) => instance as Mocker : (
							i: number,
						) => (instance as Mocker[])[i];
					total =
	...
	public setParams(value: string | TObj): TObj | never {
		if (this.mockit) {
			...
			return instance.setParams(
				typeof value === "string" ? Dispatcher.parse(value, {
					mockit: instance,
				}) : value,
			);
		} else {
				...
				if (refName) {
					if (hasOwn(namedData, refName)) {
						...
						if (isArray(namedData[refName])) {
							...
							namedData[refName] = [namedData[refName] as TemplateData, instanceData];
						}
					} else {
...
class Depender {
	...
	private dependValuedUpdateFns: TObj<((value: TDynamicDependValue) => void)[]> =
		{};
	// dynamic execute fns
	...
	public addNode(
		...
		const args: TDynamicDependValue[] = [];
		...
		const dynamic: TDynamicExecuteFn = (
			this.dynamicExecuteFns[currentId] = {
				checked: false,
				fn: () => {
					if (!dynamic.checked) {
						let index = 0;
						for (const arg of args) {
							if (!hasOwn(arg, "value")) {
								throw new Error(
									`The path ${currentId} depend a path of '${dependIds[
										index
									]}' not make a value yet, make sure the depend path is appear before the current path.`,
								);
							}
							index++;
						}
	...
	public setRootNode(node: DependTreeNode, parent: DependTreeNode) {
		const index = this.rootNodes.findIndex(
			(curNode) => DependTreeNode.isSameNode(curNode, node),
		);
	...
	private checkLoopDependence(
		...
		if (parentNodes.find((node) => node.id === curId)) {
			throw new Error(
				`The depend path of '${parentNodes[parentNodes.length - 1]
					.id}' and '${curNode.id}' cause a loop dependencies.`,
			);
		}
	}
	// loop the tree node
...
export default class SuchMocker<T = unknown> {
   ...
   * @memberof Such
   */
	public checkKeys(keys: ValueOf<Pick<IAInstanceOptions, "keys">>):
		| void
		| never {
			...
			for (const path in keys) {
				if (hasOwn(keys, path)) {
					if (hasOwn(fields, path)) {
						...
						const config = curInstance?.isEnum ? enumConfig : fields[
							path
						] || {};
						...
						if (hasMin || hasMax) {
							...
							const checkMinOrMax = (val: unknown, name: "min" | "max"):
								| void
								| never => {
...
export class Such {
   ...
   */
	...
	public store<T extends keyof Store>(name: T[]): Pick<Store, T>;
	public store<T extends keyof Store>(name: T, ...args: T[]): Pick<Store, T>;
	public store<T extends keyof Store>(name: T | T[], ...args: T[]):
		| Store[T]
		| Pick<Store, T> {
   ...
   */
	public clearStore(
		options?: {
			reset?: boolean;
			exclude?: TStoreAllowedClearFileds[] | TStoreAllowedClearFileds;
		},
	) {
   ...
   */
	...
	public define(
		...
		const initProcess = function (this: Mockit, genFn?: GeneratorFunction) {
			...
			const params =
				typeof param === "string" ? Dispatcher.parse(param, {
					mockit: this,
				}) : {};
			if (isFn(genFn)) {
		...
		if (hasOwn(mockits, type) || hasOwn(alias, type)) {
			...
			if (isExtend) {
				...
				const BaseClass = (
					hasNs ? mockits[realBaseType] || globalStoreData.mockits[
						globalStoreData.alias[baseType] || baseType
					] : mockits[realBaseType]
				) as unknown as typeof BaseExtendMockit;
				if (!BaseClass) {
					throw new Error(
			...
			if (!hasOwn(mockits, long)) {
				throw new Error(
					`You can't set an alias "${short}" for the data type of "${long}" which is not defined${
						this
							.hasNs ? ", you can only set alias name for your own defined types" : ""
					}.`,
				);
			}
			if (this.hasNs) {
	...
	public config(config: TSuchSettings): void {
		...
		Object.keys(lastConf).map((key: keyof TSuchSettings) => {
			...
			Object.keys(conf).map((name: keyof typeof conf) => {
				...
				const args = utils.isArray(value) ? value : (
						...
						if (mockit) {
							...
							if (match) {
								...
								if (mockitClass) {
									...
									// no mockit access
									throw new Error(
										`[index:${storeIndex}] The data type expression in template literal \`${match[
											0
