...
const createStore = (namespace?: string): Store => {
	const fn = (
		(name: string, value: unknown, assignType: AssignType): void => {
			...
			if (isEqualNameDiffType) {
				...
				throw new Error(
					`The field '${name}' that assigned as a ${names[
						0 ^ reversed
			...
			if (hasOwn(assignTypes, name)) {
				...
				if (lastAssignType < origAssignType) {
					throw new Error(
						`The variable '${name}' has ever assigned with a assign type '${AssignType[
							origAssignType
						]}', can't re assigned with a less strictly type '${AssignType[
							lastAssignType
						]}' again.`,
					);
				}
				switch (lastAssignType) {
					case AssignType.MustNotOverride:
						throw new Error(
							`The variable '${name}' with a assign type '${AssignType[
								lastAssignType
							]}' has ever exist,can't assign it again.`,
						);
	...
	const clearMockits = namespace ? (reset?: boolean) => {
		if (reset) {
			fn.mockits = {};
		} else {
	...
	const clearByInit = (...excludes: TStoreAllowedClearFileds[]) => {
	...
	const clearOrigin = (...excludes: TStoreAllowedClearFileds[]) => {
		// clear the assigned variables
		(["fns", "vars"] as const).forEach((key) => {
			if (!excludes.includes(key)) {
