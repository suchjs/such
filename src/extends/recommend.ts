 ...
 */
const protocols = [
	"https", "http", "ftp", "telnet", "mailto", "gopher", "file",
];
const tlds = [
	"com", "net", "org", "top", "wang", "ren", "xyz", "cc", "co", "io", "cn",
...
const confs: TSuchSettings = {
     ...
     */
		...
		color$rgba(_: TSuchInject, such: Such): string {
			...
			return (
				`rgba(${[instance.a(), instance.a(), instance.a(), opacity || 0].join(
					",",
				)})`
			);
		},
		...
		color$hsla(_: TSuchInject, such: Such): string {
			...
			const opacity = such.as(":number[0,1]:%.2f");
			return (
				`hsla(${[highlight, instance.a(), instance.a(), opacity || 0].join(
					",",
				)})`
			);
		},
