...
const handle$size = function ($size: IPPSize) {
  if (!$size) {
    return;
  }
...
export default class ToString extends Mockit<string> {
  ...
  public init(): void {
    ...
    // $size Rule
    this.addRule('$size', handle$size);
  }
  // generate
  public generate(options: TSuchInject): string {
    const params = this.params;
    const { $length, $size } = params;
    const { least, most } = Object.assign({ least: 1, most: 100 }, $length, options.param?.$length);
    const { range } = Object.assign({
      range: [[32, 126]],
    }, $size, options.param?.$size?.range ? handle$size(options.param.$size as IPPSize) : {});
